...
>(
  ...
  const thunk = createAsyncThunk(typePrefix, wrapper) as AsyncThunkWithReducers<
    Returned,
    State,
    ThunkArg
  >;
...
type RecordElement<M extends Record<string, unknown>> = M extends Record<
  string,
  infer ElementType
>
...
type StateEntity<State extends EntityState<unknown>> = RecordElement<
  State['entities']
>;
...
> =
  ...
  ((
    ...
    PayloadAction<Returned, string, { arg: ThunkArg }, Error | string>
  > & { type: string }) &
